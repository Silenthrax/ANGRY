    ...
    async def start_single_client(self, client_num, client):
        ...
        try:
            ...
            try:
                ...
                LOGGER(__name__).error(f"Error setting client attributes for Assistant {client_num}: {e}")
                return False
                
        except FloodWait as e:
            try:
                sleep_time = float(e.value) if hasattr(e, 'value') and isinstance(e.value, (int, float)) else 30
                LOGGER(__name__).warning(f"FloodWait for Assistant {client_num}: {sleep_time}s")
                await asyncio.sleep(sleep_time)
                return await self.start_single_client(client_num, client)
            except Exception:
                LOGGER(__name__).warning(f"FloodWait error for Assistant {client_num}, waiting 30s")
            ...
            try:
                if client and hasattr(client, 'is_connected'):
                    ...
                    try:
                        ...
                        LOGGER(__name__).warning(f"Error checking connection for client {client_num}: {e}")
                        # Try to stop anyway
                        stop_tasks.append(client.stop())
            except Exception as e:
                LOGGER(__name__).warning(f"Error preparing to stop client {client_num}: {e}")
